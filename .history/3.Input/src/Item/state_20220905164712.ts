import { atom, selector, atomFamily, selectorFamily, equalSelectorFamily } from "recoil";
...
export const doublePositionFamily = equalSelectorFamily<any,  {modelKey:string, pid: string}>({
    key: "positionSelectFamilyEqual",
    get:
		({modelKey, pid}) =>
		({ get }) => {
			return get(modelByKey(modelKey))[pid];
